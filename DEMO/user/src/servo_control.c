...
static uint8_t roundabout_entry_pending = 0;      // 环岛检测锁存，防止计时过程中信号丢失
...
{			
    ...
    // 锁存检测结果，防止在延时计时期间 roundabout_detected 因门限抖动被清零
    if(roundabout_detected)
    {
        roundabout_entry_pending = 1;
    }

    // 检测到环岛开始计时（使用锁存标志，保证延时完成后必定触发强制左转）
    if(roundabout_entry_pending && !roundabout_force_active)
    {
        ...
        else
        {
            ...
            roundabout_entry_pending = 0;  // 已经进入强制转向，无需继续等待
        }
    }
    else if(!roundabout_entry_pending)
    {
        roundabout_detect_timer = 0;
    }
    ...
    if(roundabout_force_active)
    {
        ...
        else
        {
            ...
            roundabout_entry_pending = 0;  // 强制转向结束后清空锁存，等待下一次检测
        }
