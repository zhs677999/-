...
bool BOARD_SDCardGetDetectStatus(void)
{
    ...
    if(gpio_get_level(BOARD_SDMMC_SD_CD_GPIO_PIN) == BOARD_SDMMC_SD_CD_INSERT_LEVEL)
    {
        return true;
    }
...
//        HAL_GpioInit(s_CardDetectGpioHandle, &sw_config);
        gpio_init(BOARD_SDMMC_SD_CD_GPIO_PIN, GPI, 0, GPI_PULL_UP);
...
//    HAL_GpioInit(s_PowerResetGpioHandle, &sw_config);
    ...
    gpio_init(BOARD_SDMMC_SD_POWER_RESET_GPIO_PIN, GPO, 0, GPO_PUSH_PULL);
}
...
void BOARD_SDCardPowerControl(bool enable)
{
    if (enable)
    {
        ...
        gpio_set_level(BOARD_SDMMC_SD_POWER_RESET_GPIO_PIN, 0);
    }
    else
    {
        //HAL_GpioSetOutput(s_PowerResetGpioHandle, 1);
        gpio_set_level(BOARD_SDMMC_SD_POWER_RESET_GPIO_PIN, 1);
    }
}
